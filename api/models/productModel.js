...
function getAllProducts(page = 1, limit = 10) {
  ...
  const total = dbInstance.prepare("SELECT COUNT(*) as count FROM products").get().count;
  const offset = (page - 1) * limit;
  const data = dbInstance.prepare("SELECT * FROM products LIMIT ? OFFSET ?").all(limit, offset);
  return { total, data };
}
...
module.exports = { getAllProducts, getDb, closeDb };
